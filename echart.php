...
header("Content-Type: text/html; charset=utf-8");
  ...
  //echo $type;
    ...
    $sql = "select `期间`, sum(`销售数量(MT)`) as '销量' ,ROUND(avg(`销售收入`)/avg(`销售数量(MT)`),2) as '吨均售价' from `maindata` where `物料描述` IN (select `物料` from `filter` where `物料类型`= '$types' ) and `会计年度` = $year group by `期间` order by `期间`";
